...
# an Interactive-CAsT deployment (docker compose only, not K8S)
# 
# Supported operations:
#  - create a VM, install dependencies, clone the repo, build, deploy
...
exit_handler() {
    ...
    then
        echo_color "\n\n*** The selected deployment steps were NOT successfully completed!\n" "${YELLOW}"
        echo_color "*** Check the output above for error messages\n" "${YELLOW}"
    else
        echo_color "\n\nThe selected deployment steps were successfully completed!\n" 
    fi
}
...
does_firewall_rule_exist() {
    # Checks if a named firewall rule already exists on the default network
    #
    # Arguments:
    #   $1 = name of the rule
    #
    # Return value: 0 if rule exists, 1 if not
    resp=$(gcloud compute firewall-rules list --filter=name="${1}" 2> /dev/null | grep "${1}")
    is_response_not_empty "${resp}"
}
...
check_and_enable_required_services() {
    # For each of the services required for a deployment, check if it 
...
if [[ $# -lt 1 ]]
then
    echo "Usage: oat_gcp_config.sh <create|tests|logs|cmd|delete>"
    echo ""
    echo "Available commands:"
    echo -e "   ${GREEN}create${NC}: create a new VM instance and start a deployment"
    ...
    echo -e "   ${GREEN}delete${NC}: delete an active VM instance"
    exit 0
fi
...
then
    ...
    then
        if ! gcloud compute firewall-rules delete "${rule_name}" --quiet
        then
            echo_color "> Failed to delete firewall rule\n" "${YELLOW}"
        fi
    fi
elif [[ "${1}" == "create" ]]
then
    ...
    if does_firewall_rule_exist "${rule_name}"
    then
        echo_color "> Firewall rule already exists\n"
    else
        echo_color "> Creating firewall rule\n"
        gcloud compute firewall-rules create "${rule_name}" --action allow --rules tcp:5000
    fi
